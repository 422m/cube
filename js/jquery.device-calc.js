  ...
  $(function() {
		...
		$.ajax({
		  ...
		  success: function(result){
				...
				{
					$.each(data.devices, function(index, el){
						$('<div class="item"><a href="#">Repair ' + el.name + '</a></div>').appendTo($('.burger .ajax-list')).find('a').on('click', function(){
							changeDevice(index);
							$('.burger').slideUp(500);
						});
					});
				...
				function changeModal(indexD, indexM)
				{
					//changeDevice(indexD);
						...
						$('.malfunction').slideUp(500,function(){
							...
							if(_arrModel.malfunctions)
							{
		  					...
		  					$.each(_arrModel.malfunctions, function(indexP, el){
			  					var _newMalMainTmpl = _malMainTmpl, _newMalFullTmpl = _malFullTmpl, _dataMal = this;
